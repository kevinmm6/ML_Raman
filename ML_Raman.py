...
#%% Decision Tree
def decision_tree(X,target_data):
    dt_optimization_leafs = [2,4,6,8,10,12,14,16,18] #Number of optimization tree leafs to try 
...
decision_tree(X,target_data)
# %% Random Forest 
def random_forest(X,target_data):
    rf_optimization_leafs = [20,22,24,28,30,32,34]
    for ii,target_name in enumerate(target_data):
        rfmae = [] 
        n = 0   
        y = np.array(target_data.iloc[:,ii])
